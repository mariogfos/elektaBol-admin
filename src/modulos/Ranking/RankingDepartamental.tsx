...
const RankingDepartamental = ({ extraData }: any) => {
  const [index, setIndex] = useState(0);
  const { execute, waiting, error, loaded } = useAxios();
  const [dataDpto, setDataDpto]: any = useState([]);
  const [formState, setFormState]: any = useState({
    year_week: extraData?.year_weeks?.[0]?.id,
    dpto_id: extraData?.dptos?.[0]?.id,
  });
  ...
  useEffect(() => {
    if (dataDpto?.data) {
      const sortedData:any = [...dataDpto.data].sort((a, b) => a.position - b.position);
...
export default RankingDepartamental;
