  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <h1 style={{marginBottom:16,fontSize:'var(--sXl)'}}>{event?.name}</h1>

        <div style={{display:'flex',gap:4,marginBottom:8,textAlign:'left'}}>
        ...
        <div style={{display:'flex',gap:4,marginBottom:4}}>
          <IconAffliate size={18} color="#656f78" />
          <div className="truncatedText" style={{textAlign:'start'}}> 
          {event?.attendances === 1
            ? "1 asistente registrado"
            : `${event?.attendances} asistentes registrados`}
            </div>
        </div>
      </div>
    </div>
